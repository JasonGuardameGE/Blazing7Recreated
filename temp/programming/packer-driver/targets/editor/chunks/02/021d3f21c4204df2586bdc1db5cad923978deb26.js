  ...
  return {
    ...
    execute: function () {
      ...
      _export("BasePanel", BasePanel = (_dec = ccclass("BasePanel"), _dec(_class = class BasePanel extends (_crd && BaseUI === void 0 ? (_reportPossibleCrUseOfBaseUI({
        ...
        onMaskClick(event) {
          const target = event.target; // 检查点击目标是否是当前面板或其子节点
        ...
        close(playClick = true, callback = null, closeMask = true) {
          if (playClick) {
        ...
        openWithRight() {
          const nodeWidth = this.node.getComponent(UITransform).width;
          const parentWidth = (_crd && PLK === void 0 ? (_reportPossibleCrUseOfPLK({
            error: Error()
          }), PLK) : PLK).ui.sceneWidth; // 获取Widget生效后的正确Y位置

          const widgetAdjustedY = this.node.position.y; // 右侧开启：从右侧外边缘滑入到内边缘

          const startX = (parentWidth - nodeWidth) / 2 + nodeWidth;
          const targetX = 0; //startX - nodeWidth  // 与计算规则一致
        ...
        closeWithRight(callback) {
          ...
          const targetX = (parentWidth - nodeWidth) / 2 + nodeWidth;
          tween(this.node).to(0.2, {
        ...
        openWithBottom() {
          ...
          const parentHeight = (_crd && PLK === void 0 ? (_reportPossibleCrUseOfPLK({
            error: Error()
          }), PLK) : PLK).ui.sceneHeight; // 获取Widget生效后的正确X位置

          const widgetAdjustedX = this.node.position.x; // 底部开启：从底部外边缘滑入到内边缘

          const startY = -((parentHeight - nodeHeight) / 2 + nodeHeight);
          const targetY = startY + nodeHeight; // 与计算规则一致
          // 先设置起始位置，避免在tween中设置active导致闪现
        ...
        openWithLeft() {
          ...
          const widgetAdjustedY = this.node.position.y; // 左侧开启：从左侧外边缘滑入到内边缘

          const startX = -((parentWidth - nodeWidth) / 2 + nodeWidth);
          const targetX = 0; //startX + nodeWidth  // 与Menu.ts中的计算一致
          // 先设置起始位置，避免在tween中设置active导致闪现
        ...
        openWithTop() {
          ...
          const widgetAdjustedX = this.node.position.x; // 顶部开启：从顶部外边缘滑入到内边缘
        ...
        closeWithBottom(callback) {
          ...
          const targetY = -((parentHeight - nodeHeight) / 2 + nodeHeight);
          tween(this.node).to(0.2, {
