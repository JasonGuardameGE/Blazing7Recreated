    ...
    execute: function () {
      ...
      _export("default", TicketData = class TicketData {
        ...
        updateTicketItem(data) {
          try {
            let codes = this.markWinningCells(data.codes);
            data.codes = codes;
        ...
        markWinningCells(codeStr, target = 7) {
          const nums = codeStr.split(',').map(Number); // 3x3 grid

          const g = [nums.slice(0, 3), nums.slice(3, 6), nums.slice(6, 9)]; // 默认全部不中奖

          const win = Array(9).fill(0); // 所有需要检查的线
