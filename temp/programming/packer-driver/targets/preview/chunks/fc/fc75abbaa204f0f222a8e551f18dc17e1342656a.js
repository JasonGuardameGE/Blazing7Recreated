    ...
    execute: function () {
      ...
      _export("UIRoot", UIRoot = (_dec = ccclass('UIRoot'), _dec2 = property(Node), _dec3 = property(Node), _dec(_class = (_class2 = class UIRoot extends Component {
        ...
        addScene(node) {
          ...
          var firstScene = this.scene.children[0];
          this.scene.children.length > 1 && firstScene && this.scene.removeChild(firstScene);
        }

        addPopup(node, hasMask) {
          if (hasMask === void 0) {
            hasMask = true;
          }

          var mask = this.cloneMask;
        ...
        removePanel(name) {
          var mask = this.panel.getChildByName('mask');
          mask && this.panel.removeChild(mask);

          if (typeof name == 'string') {
            var node = this.panel.getChildByName(name);
        ...
        removePopup(name) {
          var mask = this.popup.getChildByName('mask');
          mask && this.popup.removeChild(mask);

          if (typeof name == 'string') {
            var node = this.popup.getChildByName(name);
        ...
        get cloneMask() {
          var mask = instantiate(this.maskNode);
          mask.active = true;
          return mask;
        }
