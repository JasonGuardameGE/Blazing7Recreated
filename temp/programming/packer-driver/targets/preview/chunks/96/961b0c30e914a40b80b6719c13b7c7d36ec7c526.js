  ...
  return {
    ...
    execute: function () {
      ...
      _export("BGSize", BGSize = (_dec = ccclass('BGSize'), _dec2 = property(CCBoolean), _dec(_class = (_class2 = class BGSize extends Component {
        ...
        init() {
          var bg = this.node.getComponent(Sprite);

          if (!bg || !bg.spriteFrame) {
            return;
          }

          var spriteFrame = bg.spriteFrame;
          var rect = spriteFrame.getRect();
          var w = rect.width; // 图片原始宽度

          var h = rect.height; // 图片原始高度
          ...
          if (this.fixWidth) {
            ...
            var screenHeight = visibleSize.height; // 计算宽度和高度的缩放比例

            var scaleX = screenWidth / w;
            var scaleY = screenHeight / h; // 取较大的缩放比例，确保图片能够完全覆盖屏幕（可能会有部分超出）

            var scale = Math.max(scaleX, scaleY); // 设置节点宽高，保持图片比例
