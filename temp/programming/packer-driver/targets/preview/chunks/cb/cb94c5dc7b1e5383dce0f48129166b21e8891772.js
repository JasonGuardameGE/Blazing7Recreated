  ...
  return {
    ...
    execute: function () {
      ...
      _export("default", PLK = class PLK {
        ...
        static preloadPopUp() {
          // 最高优先级：WinPopUp
          var highPriorityPath = "prefab/WinPopUp";
          PLK.ui.preloadScene(highPriorityPath, progress => {// logger.log('WinPopUp 预加载进度', progress);
          }); // 低优先级：其他二级弹窗，分批静默后台预加载，避免集中卡顿

          var lowPriorityList = ["prefab/ExitPopup", "prefab/Trajectory", "menu-bundle:resources/prefab/BundleMenu"];
          var baseDelay = 800; // 毫秒，首个低优先级弹窗的延迟时间

          lowPriorityList.forEach((path, index) => {
            var delay = baseDelay * (index + 1);
            setTimeout(() => {
        ...
        static exitGame() {
          ...
          var callbackUrl = PLK.gameData.urlParams.callbackUrl;
        ...
        static loadBackground(sprite) {
          var deviceType = (_crd && DeviceUtils === void 0 ? (_reportPossibleCrUseOfDeviceUtils({
            error: Error()
        ...
        static preloadUI() {
          // 循环进行预加载
          var uiList = ["prefab/GameRecords", "prefab/CardDetail", "prefab/OtherGames", "prefab/HonorBoard"];
          uiList.forEach(item => {
        ...
        static switchGame() {
          var gameId = PLK.gameData.turnToGameId;
          var path = new URLSearchParams(window.location.search);
          path.set('gameId', gameId); // 跳转规则是把playtype正则去掉特使符号，然后转为小写

          var _gamePath = PLK.gameData.turnToPlayType.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();

          var newGamePath = PLK.gameData.config.otherGamePath + "/" + _gamePath + "?" + path.toString();
          (_crd && logger === void 0 ? (_reportPossibleCrUseOflogger({
            error: Error()
