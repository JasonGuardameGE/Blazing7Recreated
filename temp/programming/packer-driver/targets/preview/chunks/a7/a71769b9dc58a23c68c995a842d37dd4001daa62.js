System.register(["__unresolved_0", "cc", "@ge/game-common-sdk", "__unresolved_1", "__unresolved_2", "__unresolved_3"], function (_export, _context) {
  ...
  function _initSdk() {
    _initSdk = _asyncToGenerator(function* (config) {
      if (isInitialized) {
  ...
  function _request() {
    _request = _asyncToGenerator(function* (method, url, data) {
      // 保证 SDK 初始化完成后才发请求
      if (!isInitialized) {
      ...
      try {
        var sdkInstance = getSdk();
        var res = yield sdkInstance[method](url, data);
